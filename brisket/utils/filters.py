...
class Filters(object):
    ...
    def __init__(self, filter_names, verbose=False):
    ...
    def _load_filter_curves(self):
        ...
        l = np.max([len(i) for i in self.filter_names])+3
        ...
        for filt in self.filter_names:
            if filt in all_nicknames:
                ...
                if verbose: print(f"{filt}".rjust(l) + ' -> ' +  f"{all_nicknames[filt]}") #+ f"{self.filt_db[all_nicknames[filt]]['description']}".ljust(32))
            else:
                print(f"""Failed to match {filt} to any filter curve or nickname in database. Make sure it is named properly, or add it to the filter database using `brisket-filters add`""")
                sys.exit(1)
    ...
    def get_photometry(self, spectrum, redshift):
