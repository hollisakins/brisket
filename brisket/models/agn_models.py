...
class PowerlawAccrectionDiskModel(BaseAGNModel):
    ...
    def emit(self, params):
        beta, Muv = params['beta'], params['Muv'] # absolute magnitude
        sed = SED(wav_rest=self.wavelengths, flam = np.power(self.wavelengths, beta))
        sed *= np.power(10., -0.4*(sed.M_UV-Muv))
...
class AGNSlimAccretionDiskModel(BaseAGNModel):
    def __init__(self):
        # self.wavelengths = wavelengths
    def spectrum(self, model_comp):
