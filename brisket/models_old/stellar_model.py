...
class StellarModel(object):
    ...
    def __init__(self, wavelengths, parameters, logger=utils.NullLogger):
        ...
        imf_type = parameters['imf_type']
        
        self.logger = logger
        self.logger.info(f'Initializing stellar module'.ljust(50) + f'(model: {model})'.rjust(20))

        # grid = h5py.open('/path/to/model/grids/{model}/{imf}/grid.hdf5')
        # self.metallicities = grid['metallicities']
        # self.wavelengths = grid['wavelengths']
        # self.ages = grid['ages']
