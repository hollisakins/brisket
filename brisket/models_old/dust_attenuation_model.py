...
class DustAttenuationModel(object):
    ...
    def __init__(self, wavelengths, param, logger=utils.NullLogger):
        self.wavelengths = wavelengths
        self.param = param
        self.logger = logger 

        self.flag = True
        if not 'dust_atten' in list(param):
            self.logger.info("Skipping dust attenuation module")
            self.flag = False; return
            
        self.type = param['dust_atten']["type"]
        self.logger.info(f"Initializing dust attenuation module".ljust(50) + f'(type: {self.type})'.rjust(20))
